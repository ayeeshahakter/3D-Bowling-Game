...
def _reset_game():
    global ball_pos, ball_vel, ball_spinning, aim_angle_deg, power_val, power_holding, in_flight
    global trail_points, result_text, slowmo_timer_ms, bonus_pin_active, bonus_pin_index, next_bonus_check_ms
    global standing_at_throw_start, current_player, frames, frame_idx, roll_idx, throw_counter
    global pins, obstacles
    # Reset ball
    ball_pos = [0.0, LANE_Y0+60.0, BALL_RADIUS]
    ball_vel = [0.0, 0.0, 0.0]
    ball_spinning = 0.0
    ...
    # Reset pins
    for i,(px,py) in enumerate(start_positions):
        pins[i].update(x=px, y=py, vx=0.0, vy=0.0, fallen=False, falling=False, angle=0.0, 
                       fall_axis=(1,0,0), fall_speed=0.0, wobble_phase=random.uniform(0,math.tau), 
    ...
    current_player = 0
    # Reset scores and frame indices
    frames = [[[ -1, -1, -1 ] for _ in range(10)] for __ in range(max_players)]
    frame_idx = [0]*max_players
    roll_idx = [0]*max_players
    throw_counter = [0]*max_players
...
def showScreen():
    ...
    if result_text: draw_text(win_w-150, win_h-30, result_text)
    y0 = win_h-110
    ...
    draw_text(10, 20, "Space: Start/Release | 1/2/3: POV | T: Tilt | N: Players | R: Reset")
    glutSwapBuffers()

def main():
    glutInit(); glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH)
    glutInitWindowSize(win_w, win_h); glutInitWindowPosition(0, 0); glutCreateWindow(b"3D Bowling Game")
